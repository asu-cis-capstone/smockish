...
<html lang="en">
	...
	<body>
		<div id="status">
			<?php
				if (isset($_GET['lerror'])) {
					if($_GET["lerror"] == 1)
						echo '<p style="background:red">Email invalid!</p>';
					if($_GET["lerror"] == 2)
						echo '<p style="background:red">Password invalid!</p>';
					if($_GET["lerror"] == 3)
						echo '<p style="background:green">Login Success!</p>';
				}
			?>
		<div id="top">
			<table>
				<tr>
					...
					<td style="width:35%"><a class="inline" href="#signin"> Sign-In </a>|<a class="iframe" href="how_it_works.htm"> How It Works </a></td>
					</tr>
			</table>
		</div>
		...
		<div id="email">
			...
			<a class="iframe" href="http://francisogertschnig.com/Smockish/lists/?p=subscribe&id=1"><input type="button" value="get early access" /></a>
			</form>
		</div>
						...
						<tr>
							...
							<input type="text" name="refcode" placeholder="Referral Code" title="[Optional] {Limit:8} Alphanumberic Characters Only." pattern="[a-zA-Z0-9]{8,8}"/></td>
								...
								<td><label>User Type*</label><br />
									<label for="artist" class="usertypelabel">Artist</label><input class="usertypebox" type="checkbox" name="usertype[]" id="artist" value="artist"></input><br />
									<label for="artist" class="usertypelabel">Collector</label><input class="usertypebox" type="checkbox" name="usertype[]" id="collector" value="collector" ></input>
								</div> 
								</td>
							</div>

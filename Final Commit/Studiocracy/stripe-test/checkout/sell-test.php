...
<html lang="en">
	...
	<div id="artWork">
    			<table align="center">
			<?php
				$query = "SELECT u.first_name, u.last_name, A.art_id, A.artist_id, A.descrip, A.image, A.series, A.year_created, A.medium_used, A.height, A.width, A.measurement, A.weight, A.price, A.stock\n"."FROM artwork AS A, users u\n"."WHERE artist_id=u.user_id";
				...
				?>
					echo '<tr><td>
					<table class="pics">	
						<a class="pic"><img src="filesListing/<?php $row['image'] ?>"alt="" /></a>
					</table>
					</td>
					<td>
					<table class="description">
						<tr><td class="des">Description</td></tr>
						<tr><td class="input"><?php $row['descrip']?></td></tr>
					</table>
					</td>
					<td>
					<table class="information" style="text-align:left">
						<tr><td class="title">Title: <?php $row['title']?></td></tr>
						<!--
						<tr><td class="aName">Artist Name: <a href="userProfile.php?ID=<?php $row['artist_id'] ?>'.$row['first_name']." ".$row['last_name'].'</td></tr>
						...
						<tr><td><input class="button" type="button" value="Buy" style="text-align: center;"></td></tr>
						-->
					</table></td></tr>
